...
export function DatePicker({ onDateChange, currentYear, currentMonth }) {
  const years = generateYears(2026, 26);
  const months = [
...
export function Calendar({ currentMonth, currentYear, calendarTotals }) {
      ...
      {weeks.map((week, weekIndex) => (
        <div className="calendar-week" key={weekIndex}>
          {week.map((date, dateIndex) => {
            ...
            const dayTotal = calendarTotals[formattedDate] || 0;

            return (
              <Link
                to={`/budget/${formattedDate}`}
                key={dateIndex}
                className="day"
              >
                <div
                  className={`calendar-day ${
                    date.getMonth() !== currentMonth ? 'other-month-day' : ''
                  } ${
                ...
                >
                  ...
                  <div className="day-bottom">
                    <div className={`day-total ${dayTotal === 0 ? 'empty' : ''}`}>
                      {dayTotal > 0 ? `₹${dayTotal}` : 'No spend'}
                    </div>
                  </div>
                </div>
              </Link>
